...
arguments
    ...
    p (1,1) double = 1  % number of edges to be swapped in each iteration
...
improvable = 1;
u1_comb = nchoosek(1:k,p);  % set of all combinations of indices of (E')'s edges
u2_comb = nchoosek(1:m,p);  % set of all combinations of indices of (\bar{E}^V_S - E')'s edges
while 1
    ...
    for i = 1:size(u1_comb,1)
        u1 = u(u1_comb(i));  % subset of u of size p
        for j = 1:size(u2_comb,1)
            u2 = udiff(u2_comb(j));  % subset of udiff of size p
